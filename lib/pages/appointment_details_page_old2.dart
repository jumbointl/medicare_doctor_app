...
class _AppointmentDetailsPageState extends State<AppointmentDetailsPage> {
  ...
  Widget _buildVideoConsultAction() {
    ...
    if ((appointmentModel?.mustPayFirst ?? false) ||
      ...
      return ElevatedButton(
        ...
        onPressed: null,
        child: Text(
          'appointmentDetails.mustPayFirst'.tr,
    ...
    return ElevatedButton(
      ...
      onPressed: _videoLoading ? null : _handleVideoJoin,
      child: Text(
        _videoLoading ? "appointmentDetails.connecting".tr : _videoJoinButtonTitle().tr,
        style: const TextStyle(fontSize: 12, color: Colors.white),
      ),
    );
  }
  ...
  bool get _isAgoraProvider =>
      (appointmentModel?.videoProvider ?? '').trim().toLowerCase() == 'agora';

  bool get _isGoogleMeetProvider {
    final raw = (appointmentModel?.videoProvider ?? '').trim().toLowerCase();
    return raw == 'google_meet' || raw == 'googlemeet' || raw == 'meet';
  }

  String _videoJoinButtonTitle() {
    if (_isGoogleMeetProvider) {
      return 'Abrir Google Meet';
    }

    return 'appointmentDetails.joinVideo';
  }
}